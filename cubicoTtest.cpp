...
#include <fstream>
#include <vector>
#include <sstream> // Incluye esta cabecera para std::istringstream
#include <chrono>
...
vector<vector<int>> leerMatrizDesdeArchivo(const string& archivo) {
    ...
    while (getline(entrada, linea)) {
        vector<int> fila;
        istringstream ss(linea); // Usar istringstream para procesar cada línea
        while (ss >> valor) {
...
int main() {
    ...
    for (int tamaño : tamaños) {
        ...
        auto C = multiplicarMatrices(A, B);
